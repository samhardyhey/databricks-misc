...
def save_articles_metadata(articles: List[Dict], output_dir: Path) -> None:
    ...
    Args:
        ...
        output_dir: Directory to save metadata files (sources/medium/)
    """
    ...
    content_dir = output_dir / "content" / "articles"
    ...
    for idx, article in enumerate(articles, 1):
        ...
        article_file = content_dir / f"{idx:03d}_{safe_title}.json"
        with open(article_file, "w", encoding="utf-8") as f:
            json.dump(article, f, indent=2, ensure_ascii=False)
    
    logger.info(f"Saved {len(articles)} individual article references to {content_dir}")
...
def main():
    ...
    output_dir = Path(__file__).parent / "sources" / "medium"
