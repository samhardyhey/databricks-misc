...
def get_data_source() -> Literal["local", "catalog"]:
    ...
    """
    raw = os.environ.get("DOCINT_DATA_SOURCE", "auto").strip().lower()
    if raw == "catalog":
        return "catalog"
    if raw == "local":
        return "local"
...
def get_catalog_schema() -> str:
    ...
    return os.environ.get("DOCINT_CATALOG_SCHEMA", _DEFAULT_CATALOG_SCHEMA).strip()
...
def get_table_doc_pages() -> str:
    ...
    return os.environ.get("DOCINT_TABLE_DOC_PAGES", "silver_doc_pages").strip()


def get_table_doc_fields_extracted() -> str:
    """Table name for field extraction output. Override with DOCINT_TABLE_DOC_FIELDS."""
    return os.environ.get(
        "DOCINT_TABLE_DOC_FIELDS", "silver_doc_fields_extracted"
    ).strip()
