...
def train_item_similarity(product_features: pd.DataFrame, n_neighbors: int = 50, metric: str = "cosine"):
    ...
    nn = NearestNeighbors(n_neighbors=min(n_neighbors + 1, len(product_features)), metric=metric, algorithm="brute")
    nn.fit(X_scaled)
    return nn, scaler, product_features.index.tolist()


def recommend_similar_items(
    nn, scaler, product_ids: list, product_features: pd.DataFrame, product_id: str, k: int = 10
    ...
    distances, indices = nn.kneighbors(X_scaled[idx : idx + 1], n_neighbors=min(k + 1, len(product_ids)))
    out = []
...
def recommend_similar_items_batch(
    nn, scaler, product_ids: list, product_features: pd.DataFrame, query_product_ids: list[str], k: int = 10
) -> dict[str, list[tuple[str, float]]]:
    """Batch version: for each query product return top-k similar."""
    return {
        pid: recommend_similar_items(nn, scaler, product_ids, product_features, pid, k=k)
