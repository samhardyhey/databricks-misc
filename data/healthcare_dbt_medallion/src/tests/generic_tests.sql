-- Generic tests for healthcare data models

-- Test for unique identifiers
...
-- Test for valid percentage (0-100)
...
-- Test for valid discount rate (0-1)
...
{% test referential_integrity(model, column_name, ref_model, ref_column) %}
  select {{ column_name }}
  from {{ model }}
  where {{ column_name }} not in (select {{ ref_column }} from {{ ref(ref_model) }})
{% endtest %}
