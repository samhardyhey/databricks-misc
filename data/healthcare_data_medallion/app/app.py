...
LAYERS: tuple[tuple[str, str, str], ...] = (
    (
        "raw",
        "0 · Raw (CSV load)",
        "Synthetic healthcare CSVs in `data/local/` loaded by `load_local_raw_to_duckdb.py` into schema `healthcare_dev_raw`. "
        "Table names `healthcare_*` are dbt sources (see `data/healthcare_data_medallion/src/models/sources.yml`).",
    ),
    (
        "bronze",
        "1 · Bronze (dbt)",
        "Landing / typed copy of raw sources — one bronze model per source family under `src/models/bronze/`. "
        "Schema suffix `_bronze` comes from dbt `+schema: bronze` on the `bronze` folder.",
    ),
    (
        "silver",
        "2 · Silver (dbt)",
        "Conformed entities and business rules — `src/models/silver/`. Joins, cleaning, and aggregates used downstream.",
    ),
    (
        "gold",
        "3 · Gold (dbt)",
        "Analytics- and ML-ready wide tables — `src/models/gold/`. Consumed by use-cases (reco, inventory, etc.).",
    ),
    (
        "other",
        "Other schemas",
        "Any remaining DuckDB schemas (e.g. `main`, seeds/snapshots if present).",
    ),
)
...
def default_duckdb_path() -> Path:
    ...
    return _repo_root() / "data" / "local" / "medallion.duckdb"
...
def main() -> None:
    ...
    st.title("Healthcare medallion — local DuckDB")
    st.caption(
        "Browse the synthetic healthcare pipeline: **CSV → raw schema → dbt bronze → silver → gold**."
    )
...
"""
    ...
    tab_labels = [label for (_, label, _) in LAYERS]
    tabs = st.tabs(tab_labels)

    for tab, (layer_id, _title, blurb) in zip(tabs, LAYERS, strict=True):
        with tab:
            st.markdown(blurb)
