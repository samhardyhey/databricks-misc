...
supply_chain_events_summary as (
    select 
        ...
        count(distinct case when sce.is_frozen_temperature then sce.event_id end) as frozen_temperature_events,
        count(distinct case when sce.is_refrigerated_temperature then sce.event_id end) as refrigerated_temperature_events,
        count(distinct case when sce.is_hot_temperature then sce.event_id end) as hot_temperature_events
...
monthly_performance as (
    select 
        ...
        count(distinct case when of.is_delivered then of.order_id end) as delivered_orders,
        count(distinct case when of.is_on_time_delivery then of.order_id end) as on_time_orders,
        count(distinct case when of.is_cancelled then of.order_id end) as cancelled_orders,
        count(distinct case when of.is_significantly_delayed then of.order_id end) as significantly_delayed_orders,
...
select 
    ...
    coalesce(sce.frozen_temperature_events, 0) as frozen_temperature_events,
    coalesce(sce.refrigerated_temperature_events, 0) as refrigerated_temperature_events,
    coalesce(sce.hot_temperature_events, 0) as hot_temperature_events,
    ...
    case 
        when sce.first_event_timestamp is not null and sce.last_event_timestamp is not null then
            date_diff(sce.last_event_timestamp, sce.first_event_timestamp, hour)
        else null
    end as fulfillment_duration_hours,
    
    -- Calculated KPIs
    case 
        when sce.total_events > 0 then round(sce.successful_events / sce.total_events * 100, 2)
        else 0
    end as event_success_rate,
    
    case 
        when sce.total_events > 0 then round(sce.failed_events / sce.total_events * 100, 2)
        else 0
    end as event_failure_rate,
    
    case 
        when sce.total_events > 0 then round(sce.warning_events / sce.total_events * 100, 2)
        else 0
    end as event_warning_rate,
    
    -- Performance classifications
    case 
        when of.is_delivered and of.is_on_time_delivery then 'Excellent'
        when of.is_delivered and of.delivery_delay_days <= 2 then 'Good'
        when of.is_delivered and of.delivery_delay_days <= 7 then 'Fair'
        when of.is_delivered then 'Poor'
        when of.is_cancelled then 'Cancelled'
        else 'In Progress'
    ...
    case 
        when sce.frozen_temperature_events > 0 or sce.refrigerated_temperature_events > 0 then 'Temperature Controlled'
        when sce.hot_temperature_events > 0 then 'Hot Storage'
        else 'Ambient'
    end as storage_requirement,
    
    -- Risk indicators
    case 
        when of.is_significantly_delayed or sce.event_failure_rate > 20 then 'High Risk'
