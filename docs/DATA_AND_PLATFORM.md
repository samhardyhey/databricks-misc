...
**Databricks Asset Bundles:** Each jobs/serving/app bundle keeps `databricks.yml` beside a **`resources/`** folder; the bundle file uses `include: resources/*.yml` so all job/endpoint/app definitions in that folder are loaded (no `../../resources/...` indirection). **Data** components use the same pattern: `data/healthcare_data_generator/bundles/{databricks.yml,resources/*.yml}` and the same for `healthcare_data_medallion`. **`workspace.root_path`** is `${var.workspace_bundle_root}/.bundle/<bundle>/<target>`; default **`workspace_bundle_root`** is `/Workspace/Users/<email>` (and **`bundle_deploying_user_name`** for `permissions`). Override with `databricks bundle deploy --var workspace_bundle_root=...` when the deploy identity uses a service-principal home or non-user path; use **`make dab-workspace-print`** for Makefile/`WORKSPACE_USER_EMAIL` hints. Targets use **`run_as.service_principal_name`** plus **`permissions`** for the deploying user and the same SP (`CAN_MANAGE` on bundle-managed resources where the bundle API supports it). **Unity Catalog** table/volume ACLs for those principals must still be applied via grants/runbooks (`make uc-foundation-deploy`, `databricks grants`, etc.)—bundle permissions do not replace UC governance.

**dbt + CLI profile alignment:** `data/healthcare_data_medallion/dbt_profiles/profiles.yml` output names **`dev-sp`**, **`test-sp`**, **`prod-sp`** match Databricks CLI profile names in **`~/.databrickscfg`**. dbt-databricks does not read that file directly: set **`DATABRICKS_HOST`** and **`DATABRICKS_TOKEN`** (or service-principal OAuth per dbt-databricks) to the same values the corresponding CLI profile would use. Update **`http_path`** to your workspace SQL warehouse ID.
...
**Document intelligence spaCy `en_core_web_sm`:** Models are **not** on PyPI; Explosion publishes **`.whl` assets on [GitHub Releases](https://github.com/explosion/spacy-models/releases)** (see [upstream README](https://github.com/explosion/spacy-models/blob/master/README.md)). Local: `pyproject.toml` optional extra `document_intelligence`, plus `use_cases/document_intelligence/ensure_spacy_model.py` — can **check/install** at runtime (`ensure_en_core_web_sm()`, or `make document-intelligence-ensure-spacy-model`). Env: `DOCINT_AUTO_INSTALL_SPACY_MODEL` (default on) and optional `DOCINT_EN_CORE_WEB_SM_WHEEL_URL`. Databricks: list the wheel in `document_intelligence_job.yml` (read-only clusters should **not** rely on runtime pip). Pinned URLs: `model_dep_urls.py`. If `github.com` is blocked, consider Hugging Face [`spacy/en_core_web_sm`](https://huggingface.co/spacy/en_core_web_sm) and verify `Requires-Dist: spacy (...)` matches your pin.
